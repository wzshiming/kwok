...
type StageController struct {
	...
	delayAdaptiveQueue                    *queue.AdaptiveQueue[resourceStageJob[*unstructured.Unstructured]]
...
func (c *StageController) Start(ctx context.Context, events <-chan informer.Event[*unstructured.Unstructured]) error {
	...
	c.delayAdaptiveQueue = queue.NewAdaptiveQueue(ctx, c.delayQueue, c.playStageWorker)
...
func (c *StageController) playStageWorker(ctx context.Context) {
	...
	for ctx.Err() == nil {
		resource, ok := c.delayAdaptiveQueue.GetOrWaitWithDone(ctx.Done())
		if !ok {
			return
		}
