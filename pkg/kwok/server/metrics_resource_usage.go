...
func (s *Server) evaluateContainerResourceUsage(resourceName string, data metrics.Data) float64 {
	...
	if err != nil {
		...
		logger.ErrorContext(s.ctx, "failed to get resource usage", "err", err, "pod", log.KRef(data.Pod.Namespace, data.Pod.Name), "container", data.Container.Name)
		return 0
	}
	if u.Usage == nil {
		return 0
	...
	if r.Expression != nil {
		...
		if err != nil {
			logger := log.FromContext(s.ctx)
			logger.ErrorContext(s.ctx, "failed to compile expression", "err", err, "expression", r)
			return 0
		}
		...
		if err != nil {
			logger := log.FromContext(s.ctx)
			logger.ErrorContext(s.ctx, "failed to evaluate expression", "err", err, "expression", r)
			return 0
		}
		return out
	}
