			...
			err = wait.For(
				func(ctx context.Context) (done bool, err error) {
					...
					if err = client.Get(ctx, ss[0].Name, ss[0].Namespace, &item); err != nil {
						logger.ErrorContext(ctx, "failed to list stage", "err", err)
						return false, nil
					}
			...
			err = wait.For(
				func(ctx context.Context) (done bool, err error) {
					...
					if err = client.Get(ctx, node.Name, node.Namespace, &item); err != nil {
						logger.ErrorContext(ctx, "failed to list node", "err", err)
						return false, nil
					}
			...
			err = wait.For(
				func(ctx context.Context) (done bool, err error) {
					...
					if err = client.Get(ctx, pod0.Name, pod0.Namespace, &item); err != nil {
						logger.ErrorContext(ctx, "failed to list pod", "err", err)
						return false, nil
					}
